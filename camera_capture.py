...
class CameraCapture:
    ...
    def capture_rtsp_images(self, project_id, rtsp_url, interval, max_count):
        ...
        try:
            while (
                self.rtsp_threads[thread_id]["running"]
                and self.rtsp_threads[thread_id]["captured_count"] < max_count
            ):
    ...
    def get_onvif_profiles(self, device_ip, device_port, username, password):
        ...
        try:
            # 构建ONVIF设备服务URL
            device_service_url = (
                f"http://{device_ip}:{device_port}/onvif/device_service"
            )

            # 获取服务信息
            services = self._get_onvif_services(device_service_url, username, password)
            media_service_url = services.get(
                "media", f"http://{device_ip}:{device_port}/onvif/media_service"
            )
    ...
    def _get_onvif_services(self, device_service_url, username, password):
        ...
        response = self._send_soap_request(
            device_service_url, soap_body, username, password
        )
    ...
    def _get_profiles(self, media_service_url, username, password):
        ...
        # 发送请求
        response = self._send_soap_request(
            media_service_url, soap_body, username, password
        )
    ...
    def capture_onvif_images(
        ...
        try:
            ...
            snapshot_uri = self.get_snapshot_uri(
                media_service_url, profile_token, username, password
            )
            if not snapshot_uri:
            ...
            while (
                self.onvif_threads[thread_id]["running"]
                and self.onvif_threads[thread_id]["captured_count"] < max_count
            ):
    ...
    def _send_soap_request(self, url, soap_body, username, password):
        ...
        try:
            ...
            response = requests.post(
                url, data=soap_body, headers=headers, auth=auth, timeout=10
            )
